...
`src/sim/flow.py` owns the canonical run sequence.
...
`src/sim/stages/estimators.py` benchmarks channel estimators only.
...
`src/sim/stages/resource_managers.py` benchmarks schedulers and power allocators.
...
`src/models/model.py` is the runtime composition root for the physical layer.
...
Shared Monte Carlo logic lives in `src/sim/stages/common.py`.
...
Current outputs are written by `src/sim/output.py`.
...
2. The runtime is organized around one shared `config.json`.
