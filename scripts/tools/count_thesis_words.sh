#!/usr/bin/env bash
# Count body-chapter words via texcount in TeX Live Docker (see thesis/notes/CONTEXT.md Word count tracking).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
THESIS="${ROOT}/thesis"
IMAGE="${THESIS_TEX_IMAGE:-texlive/texlive:latest}"
BUDGET="${THESIS}/notes/planning/word_budget.csv"
...
echo "Scope: Ch1--Ch6 prose (see thesis/notes/CONTEXT.md Word budget)"
echo
...
if [[ -f "${BUDGET}" ]]; then
  echo "Per-chapter targets (thesis/notes/planning/word_budget.csv):"
