...
def save_results_as_csv(results_data: dict, output_dir: str, filename: Optional[str] = None) -> str:
    ...
    print(f"[OK] CSV results saved to: {filename}")
    return filename
...
def load_baseline_results(baseline_path: Optional[Path] = None, project_root: Optional[Path] = None) -> Optional[dict]:
    ...
    if not baseline_path.exists():
        print(f"[WARN] Baseline results not found at: {baseline_path}")
    ...
    try:
        ...
        print(f"[OK] Loaded baseline results from: {baseline_path}")
        baseline_type = baseline.get("baseline_type", "Baseline")
        print(f"  Baseline type: {baseline_type}")
        return baseline
    except Exception as e:
        print(f"[WARN] Error loading baseline results: {e}")
        return None
