...
from src.sim.stages.common import MIN_RESOLVED_BIT_ERRORS, POINT_STATUS_RESOLVED, POINT_STATUS_UPPER_BOUND_ONLY


# Path to Dr. Athirah's PHY data files
_DATA_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))),
    "dr_athirah_simulation", "PHY layer",
)

# Path to Dr. Athirah's MAC and APP data files (lookup tables)
_MAC_APP_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))),
    "dr_athirah_simulation", "MAC and APP layer",
)
...
def run_jidd_scma_stage(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    from src.athirah.simulation import run_jidd_scma
    ...
    if jidd_cfg.get("mac_enabled", False):
        from src.athirah.mac.factory_sim import run_factory_mac_sim
    ...
    if jidd_cfg.get("app_enabled", False):
        from src.athirah.app.e2e_metrics import run_app_fec_sim
